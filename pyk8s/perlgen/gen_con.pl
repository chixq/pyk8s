...
sub get_top_item
{
    ...
    {
        chomp();
        my $prefix = $struct_strack[-1];
        if(/^\s*\"(\w+)\":\s*\{}/){
            my $top_struct=$prefix."_".$1."-S";
...
sub gen_py_source
{
    ...
    while(my($k,$v)=each %struct_items){
        ...
        while(my($item,$type)=each %items_map){
            my @strs=split('_',$item);
            my $item_no_prefix = $strs[-1];
            print TAR "            \'$item_no_prefix\':None,\n";
        }
        print TAR "         }\n";
        print TAR "\n";
        ...
        while(my($item,$type)=each %items_map){
            my @strs=split('_',$item);
            my $item_no_prefix = $strs[-1];
            my $single_item_no_prefix=$item_no_prefix;
            $single_item_no_prefix =~ s/s$//g; 
            if($type eq 'A')
            {
                my $acture_item= $item_no_prefix;
                if($item_no_prefix eq "items"){
                   $acture_item = $k;
                   $acture_item =~ s/s$//g;
                }
                else
                ...
                my $single_item = $item_no_prefix;
                $single_item =~ s/s$//g;
                print TAR "        i=0\n";
                print TAR "        for $single_item in self.$item_no_prefix:\n";
                print TAR "            params['".$item_no_prefix."'][i]=".$single_item.".toDict();\n";
                print TAR "            i=i+1;\n"
            }
            elsif($type eq 'S')
            {
                if(exists $struct_items{$item}){
                    print TAR "        params['$item_no_prefix']=self.$item_no_prefix.toDict();\n";
                }
            }
        }
        print TAR "        \n";
        ...
        while(my($item,$type)=each %items_map){
            ...
            if($type eq "S")
            {
                ...
                print TAR "                $item_no_prefix=".ucfirst($item).".newFromDict(data.get(\'$item_no_prefix\', {})),\n";
            }
            elsif($type eq "C")
            {
                print TAR "                $item_no_prefix=data.get('$item_no_prefix', None),\n";
            }
            elsif($type eq "A")
            {
                my $single_item = $item_no_prefix;
                ...
                print TAR "                ".$item_no_prefix." = [".ucfirst($single_item).".newFromDict($single_item_no_prefix) for $single_item_no_prefix in (data.get('$item_no_prefix',{}) if (data.get('$item_no_prefix',{}) is not None) else {})],\n";
            }
        }
        print TAR "            )\n";
        print TAR "\n";
